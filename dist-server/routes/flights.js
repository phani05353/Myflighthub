...
flightsRouter.post('/', async (req, res) => {
    ...
    try {
        ...
        if (!raw)
            return res.status(404).json({ message: `Flight ${iata} not found` });
        const row = mapToRow(raw, uuid(), new Date().toISOString());
        db.prepare(`INSERT INTO tracked_flights
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(row.id, row.flight_iata, row.airline_name, row.origin, row.destination, row.aircraft, row.scheduled_departure, row.scheduled_arrival, row.actual_departure, row.actual_arrival, row.delay_minutes, row.status, row.next_day_arrival, row.added_at, row.updated_at);
