...
import { lookupFlight, mapToRow, deriveStatus } from './aviation.js'
...
export async function refreshActive() {
  ...
  for (const row of active) {
    try {
      ...
      console.error(`Refresh failed for ${row.flight_iata}:`, (e as Error).message)
    }
  }
}

async function refreshOne(existing: FlightRow) {
  ...
  getDb()
    ...
    .run(
      ...
      updated.scheduled_arrival,
      updated.actual_departure,
      updated.actual_arrival,
      updated.delay_minutes,
      updated.status,
      updated.next_day_arrival,
      updated.updated_at,
      existing.id
    )
...
function emitAlert(flightIata: string, newStatus: string, delay: number) {
  const db = getDb()
  let type: string
  let message: string
  ...
  db.prepare(
    `INSERT INTO alerts (id, flight_iata, type, message, created_at) VALUES (?,?,?,?,?)`
  ).run(uuid(), flightIata, type, message, new Date().toISOString())
}
