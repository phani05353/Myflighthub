...
import { initDb, getDb, type FlightRow } from './db.js'
...
app.get('/api/airports', (_req, res) => {
  ...
  type Bucket = { deps: FlightRow[]; arrs: FlightRow[] }
  const map = new Map<string, Bucket>()

  const bucket = (code: string): Bucket => {
    if (!map.has(code)) map.set(code, { deps: [], arrs: [] })
    return map.get(code)!
  }

  for (const f of flights) {
    if (f.origin) bucket(f.origin).deps.push(f)
    if (f.destination) bucket(f.destination).arrs.push(f)
  }
