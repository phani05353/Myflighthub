...
export async function lookupFlight(flightIata: string): Promise<RawFlight | null> {
  const key = process.env.AVIATIONSTACK_API_KEY
  if (!key) {
    throw new Error(
      'AVIATIONSTACK_API_KEY is not set. Get a free key at aviationstack.com and add it to .env'
    )
  }
...
function crossesMidnight(dep: string | null, arr: string | null): boolean {
...
export function deriveStatus(
  ...
  if (raw === 'scheduled' && depScheduled) {
    ...
    if (mins > 0 && mins <= 45) return 'boarding'
  }

  return 'on-time'
}

export function mapToRow(raw: RawFlight, id: string, addedAt: string) {
